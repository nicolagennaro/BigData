...
exception of five users, all of them comes from USA, and in particular from the continent, as we can see from the
...
The mean is a little bit higher than 3, that would have been the mean value in the case of a uniform distribution 
...
Maybe instead of considering the mean vote as measure it is convenient to look also at the number (or better the
proportion) of movies seen by a particular gender, of course in function of the movie genre.
...
As one can see, in the proportion there is in fact a remarkable difference between men and women. In fact now it's more
evident, for example, that female have watched more Romance and Musical movie, while male more Action, Horror, Western and War
Movies. Strangely the mean rating seems to carry not so much informations about the kind of movie a particular user will
watch.
...
Let's also see the proportion of counts for each age category:
...
Again the proportion yields more difference that the mean vote. It appears that old people have watched in proportion more
Westerns and War movies that the other users, while as expected the <18 category is the one that has seen more movies for
children.
...
- According to the mean rating, gender is slightly related to the movies' genre but only for some of them 
(especially Musicals and Childrens' movies), but, looking at the count of seen movies, a pattern is present.
